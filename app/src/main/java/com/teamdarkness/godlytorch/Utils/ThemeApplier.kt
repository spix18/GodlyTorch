...
object ThemeApplier {
    fun applySystemBars(window: Window, theme: AppTheme) {
        window.statusBarColor = theme.backgroundColor
        window.navigationBarColor = theme.backgroundColor
        WindowInsetsControllerCompat(window, window.decorView).apply {
            isAppearanceLightStatusBars = theme.lightSystemBars
            isAppearanceLightNavigationBars = theme.lightSystemBars
        }
    }
}
