...
/** White-only torch (dual-tone devices). */
class WhiteTileService : TorchTileBase() {
    override val tileStatusKey = "whiteTileStatus"
    override val tileOnKey = Prefs.TILE_WHITE_ON
    override val tileNameKey = Prefs.TILE_WHITE_NAME
    override val tileStateKey = Prefs.TILE_WHITE_STATE
    override val defaultLabel = "White Torch"
    override val supportsSingleTone = false
    override val mutexes = listOf(
        Mutex(Prefs.TILE_YELLOW_ON, "Yellow Torch tile is active. You can use one tile at a time only."),
